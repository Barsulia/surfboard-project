...
const countSectionPosition = sectionNum => {
  ...
  if (isNaN(position)) {
    console.error("в countSectionPosition передано не верное значение");
    return 0;
  }
...
const changeFixedMenuTheme = sectionNum => {
...
const resetActiveClassForItem = (items, itemNum, activeClass) => {
...
const performTransition = sectionNum => {
  ...
  changeFixedMenuTheme(sectionNum);
  ...
  setTimeout(() => {
    inScroll = false;
    resetActiveClassForItem(sideMenuItems, sectionNum, "fixed-menu__item--active");
